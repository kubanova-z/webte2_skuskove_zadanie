...
@section('content')
    <div class="max-w-2xl mx-auto mt-8 bg-white p-6 shadow rounded">
        <h2 class="text-xl font-bold mb-4">{{ __('messages.resize_pdf_title') }}</h2>
        ...
        <form method="POST" action="{{ route('pdf.resize-pages.process') }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label class="block font-medium text-gray-700">{{ __('messages.pdf_file') }}:</label>
                <input type="file" name="pdf" required class="mt-1 block w-full">
            </div>

            <div class="mb-4">
                <label class="block font-medium text-gray-700">{{ __('messages.page_format') }}:</label>
